...
function diffTable(values,columns){
  ...
  for(let i=0;i<columns.length-1;i++) d1.push(values[columns[i+1]]-values[columns[i]]);
  for(let i=0;i<d1.length-1;i++) d2.push(d1[i+1]-d1[i]);

  return `
    <table class="sequence-table">
      <tr><th>n</th>${columns.map(n=>`<th>${n}</th>`).join("")}</tr>
      <tr><th>Folge</th>${columns.map(n=>`<td>${values[n]}</td>`).join("")}</tr>
      <tr><th>1. Differenzfolge</th>${d1.map(v=>`<td>${v}</td>`).join("")}<td></td></tr>
      <tr><th>2. Differenzfolge</th>${d2.map(v=>`<td>${v}</td>`).join("")}<td></td><td></td></tr>
    </table>
  `;
}
